  ...
  return (
    <main style={{ minHeight: '100vh', background: '#0F1208' }}>
      ...
      <div className="photo" style={{ height: '58vh', position: 'relative', display: 'flex', alignItems: 'flex-end', paddingBottom: 72, paddingTop: 72 }}>
        ...
        <div style={{ position: 'relative', zIndex: 2, padding: `0 ${P}`, maxWidth: 1440, width: '100%', margin: '0 auto' }}>
          ...
          <h1 style={{ fontSize: 'clamp(36px, 6vw, 80px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', lineHeight: 1.05, maxWidth: 680 }}>Every feast<br />tells a story.</h1>
        </div>
      </div>

      {/* Opening prose */}
      <section style={{ padding: `${PY} ${P}`, maxWidth: 1440, margin: '0 auto', display: 'flex', gap: 80, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        <div style={{ flex: .55, minWidth: 'min(280px, 100%)' }}>
          ...
          <p style={{ fontSize: 'clamp(18px, 2.5vw, 28px)', lineHeight: 1.45, fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 28 }}>From the moment you arrive, the air changes.</p>
        ...
        <div style={{ flex: .45, minWidth: 'min(260px, 100%)' }}>
      ...
      <section style={{ padding: `${PY} ${P}`, maxWidth: 1440, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 64 }}>Your Evening,<br />Step by Step</h2>
          ...
          {TIMELINE.map((item, i) => (
            <div key={item.n} style={{ display: 'flex', gap: 40, alignItems: 'flex-start', paddingBottom: i < 3 ? 52 : 0, borderLeft: i < 3 ? '1px solid #2A3020' : 'none', marginLeft: 20, paddingLeft: 48, position: 'relative', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: 'min(240px, 100%)' }}>
                ...
                <h3 style={{ fontSize: 'clamp(18px, 3vw, 24px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 12 }}>{item.title}</h3>
                <p style={{ fontSize: 15, lineHeight: 1.75, color: '#A89B80', maxWidth: 520 }}>{item.desc}</p>
              </div>
              <div className="photo timeline-photo" style={{ width: 'min(240px, 100%)', flexShrink: 0, aspectRatio: '4/3', borderRadius: 4, overflow: 'hidden' }}>
      ...
      <section style={{ padding: `${PY} ${P}`, maxWidth: 1440, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 48 }}>Two ways to gather</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }} className="col1 format-grid">
          <div style={{ background: '#1E2418', border: '.5px solid #2A3020', borderRadius: 12, padding: 'clamp(24px, 4vw, 40px)', borderTop: '3px solid #C8891F' }}>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.18em', textTransform: 'uppercase', color: '#C8891F', marginBottom: 16 }}>Type A</p>
            <h3 style={{ fontSize: 'clamp(20px, 3vw, 28px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 20 }}>Ethnic Food Showcase</h3>
          ...
          <div style={{ background: '#252C1A', border: '.5px solid #2A3020', borderRadius: 12, padding: 'clamp(24px, 4vw, 40px)', borderTop: '3px solid #A33D21' }}>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '.18em', textTransform: 'uppercase', color: '#A33D21', marginBottom: 16 }}>Type B</p>
            <h3 style={{ fontSize: 'clamp(20px, 3vw, 28px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 20 }}>Old vs New</h3>
      ...
      <section style={{ padding: `${PY} ${P}`, background: '#1E2418' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', gap: 80, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: .55, minWidth: 'min(280px, 100%)' }}>
            ...
            <h2 style={{ fontSize: 'clamp(28px, 4vw, 40px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 24, lineHeight: 1.1 }}>Take the table<br />home with you.</h2>
      ...
      <section style={{ padding: `${PY} ${P}`, textAlign: 'center', background: '#0F1208' }}>
        <h2 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontFamily: "'Yeseva One',serif", color: '#EFE8D5', marginBottom: 16 }}>Ready to come to the table?</h2>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => onNav('checkout')} style={{ background: 'transparent', color: '#EFE8D5', border: '1px solid rgba(239,232,213,.4)', borderRadius: 2, padding: '16px 36px', fontSize: 12, fontWeight: 600, letterSpacing: '.10em', textTransform: 'uppercase', cursor: 'pointer' }}>
            Get Tickets Now
          </button>
        </div>
      </section>
